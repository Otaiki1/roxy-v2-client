...
function accumulateEarnedPoints(
    user: string,
    opponent: string,
    startEventId: number = 1,
    createFirstEvent: boolean = false
) {
    ...
    if (createFirstEvent) {
        simnet.callPublicFn(
            contractName,
            "create-event",
            [Cl.uint(eventId), Cl.stringAscii(`Event ${eventId}`)],
            deployer
        );
    }
    ...
    simnet.callPublicFn(
        contractName,
        "stake-yes",
        [Cl.uint(eventId), Cl.uint(stakeAmount)],
        accounts.get(user)!
    );
    simnet.callPublicFn(
        contractName,
        "stake-no",
        [Cl.uint(eventId), Cl.uint(stakeAmount)],
        accounts.get(opponent)!
    );
    simnet.callPublicFn(
        contractName,
        "resolve-event",
        [Cl.uint(eventId), Cl.bool(true)],
        deployer
    ); // YES wins
    const claim1Result = simnet.callPublicFn(
        contractName,
        "claim",
        [Cl.uint(eventId)],
        accounts.get(user)!
    ...
    if (claim1Result.result.type === "err") {
        throw new Error(
            `Event 1 claim failed: ${JSON.stringify(claim1Result.result)}`
        );
    }
    eventId++;
    ...
    for (let i = 0; i < 32; i++) {
        ...
        const userStakeResult = simnet.callPublicFn(
            contractName,
            "stake-yes",
            [Cl.uint(eventId), Cl.uint(stakeAmount)],
            accounts.get(user)!
        ...
        if (userStakeResult.result.type === "err") {
            throw new Error(
                `Event ${eventId} user stake failed: ${JSON.stringify(
                    userStakeResult.result
                )}`
            );
        }
        const opponentStakeResult = simnet.callPublicFn(
            contractName,
            "stake-no",
            [Cl.uint(eventId), Cl.uint(stakeAmount)],
            accounts.get(opponent)!
        ...
        if (opponentStakeResult.result.type === "err") {
            throw new Error(
                `Event ${eventId} opponent stake failed: ${JSON.stringify(
                    opponentStakeResult.result
                )}`
            );
        }
        ...
        simnet.callPublicFn(
            contractName,
            "resolve-event",
            [Cl.uint(eventId), Cl.bool(userWins)],
            deployer
        );
        if (userWins) {
            ...
            if (claimResult.result.type === "err") {
                throw new Error(
                    `Event ${eventId} claim failed: ${JSON.stringify(
                        claimResult.result
                    )}`
                );
            }
            ...
            if (claimResult.result.type === "err") {
                throw new Error(
                    `Event ${eventId} opponent claim failed: ${JSON.stringify(
                        claimResult.result
                    )}`
                );
            }
...
describe("Roxy Contract Tests", () => {
    ...
    describe("register", () => {
        it("should register a new user successfully", () => {
            ...
            const userPoints = simnet.getMapEntry(
                contractName,
                "user-points",
            ...
            const earnedPoints = simnet.getMapEntry(
                contractName,
                "earned-points",
                Cl.principal(address1)
            );
            ...
            const userName = simnet.getMapEntry(
                contractName,
                "user-names",
                Cl.principal(address1)
            );
            ...
            const usernameMapping = simnet.getMapEntry(
                contractName,
                "usernames",
                Cl.stringAscii("alice")
            );
    ...
    describe("create-event", () => {
        ...
        it("should create an event successfully (admin only)", () => {
            ...
            const event = simnet.getMapEntry(
                contractName,
                "events",
                Cl.uint(1)
            );
    ...
    describe("stake-yes", () => {
        ...
        it("should stake YES successfully", () => {
            ...
            const stake = simnet.getMapEntry(
                contractName,
                "yes-stakes",
                stakeKey
            );
            ...
            const totalYesStakes = simnet.getDataVar(
                contractName,
                "total-yes-stakes"
            );
    ...
    describe("stake-no", () => {
        ...
        it("should stake NO successfully", () => {
            ...
            const totalNoStakes = simnet.getDataVar(
                contractName,
                "total-no-stakes"
            );
    ...
    describe("claim", () => {
        ...
        it("should claim rewards successfully when YES wins", () => {
            ...
            const userStats = simnet.getMapEntry(
                contractName,
                "user-stats",
                Cl.principal(address1)
            );
    ...
    describe("create-listing", () => {
        ...
        it("should create listing successfully when user has earned enough", () => {
            ...
            const listing = simnet.getMapEntry(
                contractName,
                "listings",
                Cl.uint(1)
            );
            ...
            const nextListingId = simnet.getDataVar(
                contractName,
                "next-listing-id"
            );
    ...
    describe("buy-listing", () => {
        ...
        it("should buy listing successfully (full purchase)", () => {
            ...
            const bobPoints = simnet.getMapEntry(
                contractName,
                "user-points",
            ...
            const protocolTreasury = simnet.getDataVar(
                contractName,
                "protocol-treasury"
            );
    ...
    describe("create-guild", () => {
        ...
        it("should create guild successfully", () => {
            ...
            const guild = simnet.getMapEntry(
                contractName,
                "guilds",
                Cl.uint(1)
            );
            ...
            const isMember = simnet.getMapEntry(
                contractName,
                "guild-members",
                memberKey
            );
            ...
            const nextGuildId = simnet.getDataVar(
                contractName,
                "next-guild-id"
            );
    ...
    describe("deposit-to-guild", () => {
        ...
        it("should deposit to guild successfully", () => {
            ...
            const deposit = simnet.getMapEntry(
                contractName,
                "guild-deposits",
                depositKey
            );
    ...
    describe("guild-stake-yes", () => {
        ...
        it("should stake YES successfully", () => {
            ...
            const guildStake = simnet.getMapEntry(
                contractName,
                "guild-yes-stakes",
                guildStakeKey
            );
            ...
            const totalGuildYesStakes = simnet.getDataVar(
                contractName,
                "total-guild-yes-stakes"
            );
    ...
    describe("guild-stake-no", () => {
        ...
        it("should stake NO successfully", () => {
            ...
            const totalGuildNoStakes = simnet.getDataVar(
                contractName,
                "total-guild-no-stakes"
            );
    ...
    describe("guild-claim", () => {
        ...
        it("should claim guild rewards successfully", () => {
            ...
            simnet.callPublicFn(
                contractName,
                "guild-stake-yes",
                [Cl.uint(1), Cl.uint(1), Cl.uint(200)],
                address2
            );
            simnet.callPublicFn(
                contractName,
    ...
    describe("mint-admin-points", () => {
        ...
        it("should mint points to admin successfully", () => {
            ...
            const adminPoints = simnet.getMapEntry(
                contractName,
                "user-points",
    ...
    describe("buy-admin-points", () => {
        ...
        it("should buy points from admin successfully", () => {
            ...
            const buyerPoints = simnet.getMapEntry(
                contractName,
                "user-points",
                Cl.principal(address1)
            );
            ...
            const treasury = simnet.getDataVar(
                contractName,
                "protocol-treasury"
            );
    ...
    describe("get-transaction-log", () => {
        ...
        it("should return transaction log", () => {
            ...
            const transactionLog = simnet.getMapEntry(
                contractName,
                "transaction-logs",
                Cl.uint(1)
            );
